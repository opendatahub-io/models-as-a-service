...
Platform administrators can bulk revoke API keys by **user**, by **subscription**, or both. A **dry-run** mode is available to preview which keys would be affected before committing.
...
Preview which keys would be revoked without actually revoking them. The response includes the count and list of key IDs that would be affected. Set `dryRun: true` with any scope (username, subscription, or both).
...
|-------|----------|-------------|
| `username` | At least one of `username` or `subscription` | Target user whose keys should be revoked. |
| `subscription` | At least one of `username` or `subscription` | MaaSSubscription name whose bound keys should be revoked. Admin-only. |
| `dryRun` | No | When `true`, returns the count of keys that would be revoked without performing any mutation. Default: `false`. |

### Authorization

| Scope | Who can do it |
|-------|---------------|
| Own keys (`username` = self) | Any authenticated user |
| Other user's keys | Admin only |
| By subscription | Admin only |
| Combined (user + subscription) | Admin only (subscription scope requires admin) |
