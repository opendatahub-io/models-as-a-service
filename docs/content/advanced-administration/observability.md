...
- **Istio Telemetry**: Adds `subscription` label to gateway latency metrics (`istio_request_duration_milliseconds_bucket`) for per-subscription P50/P95/P99 tracking
...
| **Istio Telemetry** (`istio-gateway-telemetry.yaml`) | Adds `subscription` label to `istio_request_duration_milliseconds_bucket` via the `X-MaaS-Subscription` header injected by the controller-generated AuthPolicy. Enables per-subscription latency tracking (P50/P95/P99). |
...
The Usage Dashboard supports filtering by User, Subscription, and Model variables, with pagination and a default 7-day time range. All three table columns (`authorized_hits`, `authorized_calls`, `limited_calls`) are aggregated by `(user, subscription)` so values are consistent across columns.
...
`istio_request_duration_milliseconds_bucket` is a standard Istio metric exposed by default on the gateway -- it exists even without a Telemetry resource. The Istio Telemetry resource is only needed to add the custom `subscription` label. Without it, the metric is available but cannot be broken down by subscription. This enables tracking request latency per subscription plan (e.g. free, premium, enterprise).

**How it works:**

1. The controller-generated AuthPolicy calls `/v1/subscriptions/select` to resolve the user's subscription
2. The AuthPolicy overwrites the `X-MaaS-Subscription` header with the server-resolved subscription name via Authorino's ext_authz
3. The Istio Telemetry resource extracts this header and adds it as a `subscription` label to the `REQUEST_DURATION` metric
4. Prometheus scrapes these metrics from the Istio gateway

!!! note "Security"
    The AuthPolicy overwrites the client-sent `X-MaaS-Subscription` header with the server-resolved subscription name. Even if a client sends a spoofed value, the AuthPolicy replaces it with the validated result from `/v1/subscriptions/select`. This prevents users from manipulating metrics attribution.
...
### Perses Time Picker Limitation on Stat Panels

Three stat panels (Total Tokens, Total Requests, Total Errors) use `rate(metric[$__rate_interval]) * $__interval_ms / 1000` with `calculation: sum` to respond to the dashboard time picker. The **Success Rate** panel uses raw cumulative counters with `calculation: last-number` because a rate-based ratio shows 0% when traffic stops.

| Limitation | Details |
|------------|---------|
| **No `$__range` variable** | Unlike Grafana, Perses does not expose a variable for the full dashboard time range. The workaround uses `$__rate_interval` and `$__interval_ms` to approximate per-step increases. |
| **Bursty traffic inaccuracy** | When traffic is concentrated in short bursts, the rate window can be larger than the burst, causing slight over- or under-counting depending on step size. With continuous production traffic this does not occur. |
| **`offset $__interval` crashes the UI** | The ideal approach (`sum(metric) - sum(metric offset $__interval)`) triggers React error #306 in the StatChart component. |
| **`calculation: sum` sensitivity** | When all 4 stat panels use `calculation: sum` simultaneously with complex queries, the StatChart may crash. The current approach (3 panels with `sum`, 1 with `last-number`) is stable. |
| **Table panel** | Token Consumption by User uses raw cumulative counters with `calculation: last-number` — Perses tables do not support `calculation: sum`. |
...
|-------------|--------|-------|
| **Usage dashboards** (token consumption per user, per subscription, per model) | Met | Grafana (Platform Admin, AI Engineer) and Perses (Platform Admin, AI Engineer, Usage) dashboards + `authorized_hits` with `user`, `subscription`, `model`; Prometheus scrapes Limitador `/metrics`. The Usage Dashboard (Perses-only) provides a tabular per-user breakdown. |
