...
setup_route_mode() {
  ...
  if ! kubectl wait --for=condition=Programmed gateway/"$GATEWAY_NAME" -n "$GATEWAY_NAMESPACE" --timeout="${GATEWAY_TIMEOUT}s" 2>/dev/null; then
    log_warn "Gateway not Programmed after ${GATEWAY_TIMEOUT}s - if on OCP 4.20+, check for a conflicting OSSM subscription (servicemeshoperator v2 or manually installed servicemeshoperator3). Run: kubectl get subscription -A | grep servicemesh"
  else
