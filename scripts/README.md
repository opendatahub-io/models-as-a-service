...
- `ALLOWED_ROUTE_NAMESPACES` - Comma-separated list of namespaces allowed to attach HTTPRoutes to the Gateway (e.g. `"opendatahub,llm"`). Uses `from: Selector` with `matchExpressions` on `kubernetes.io/metadata.name`. Takes precedence over `NAMESPACE_SELECTOR_LABELS`.
...
> **Important for MaaS:** HTTPRoutes are created in the application namespace (`opendatahub` / `redhat-ods-applications`) and typically in model namespaces (e.g. `llm`), not in `openshift-ingress`. Set `ALLOWED_ROUTE_NAMESPACES` (or a label selector) accordingly. `deploy.sh` defaults to the app namespace and, when `MODEL_NAMESPACE` is set, includes that namespace too.
...
# MaaS on ODH — allow app + model namespaces to attach HTTPRoutes
ALLOWED_ROUTE_NAMESPACES="opendatahub,llm" \
  ./scripts/create-ai-tenant.sh myteam

# MaaS on RHOAI
ALLOWED_ROUTE_NAMESPACES="redhat-ods-applications,llm" \
  ./scripts/create-ai-tenant.sh myteam
