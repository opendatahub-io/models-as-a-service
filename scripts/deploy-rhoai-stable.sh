...
deploy_lws() {
  ...
  if [[ $lws_exists -ne "0" ]]; then
    ...
    # Still ensure the CRD is available before continuing
    if ! wait_for_crd "leaderworkersets.leaderworkerset.x-k8s.io" 60; then
      echo "  WARNING: LeaderWorkerSet CRD not ready yet, multi-node deployments may fail"
    fi
    return 0
  fi
...
EOF
  ...
  # Wait for the LeaderWorkerSet CRD to be available before continuing
  # This is required because models using multi-node deployments depend on this CRD
  if ! wait_for_crd "leaderworkersets.leaderworkerset.x-k8s.io" 120; then
    echo "  WARNING: LeaderWorkerSet CRD not ready yet, multi-node deployments may fail"
  fi
}
