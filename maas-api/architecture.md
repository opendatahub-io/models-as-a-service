...
| Endpoint              | Method | Purpose                                      | Request Headers   | Request Body      | Response                    |
|-----------------------|--------|----------------------------------------------|-------------------|-------------------|-----------------------------|
...
| `/v1/api-keys`        | POST   | Create hash-based API key (sk-oai-*)         | None              | `{"name", "description", "expiresIn"}` | API key (shown once) |
| `/v1/api-keys`        | GET    | List all API keys for user                   | None              | None              | Array of API key metadata   |
| `/v1/api-keys/{id}`   | GET    | Get specific API key by ID                   | None              | None              | API key metadata            |
| `/v1/api-keys/{id}`   | DELETE | Revoke specific API key                      | None              | None              | Revoked API key metadata (200 OK) |
| `/v1/tiers/lookup`    | POST   | Lookup tier for user groups (internal)       | None              | `{"groups"}`      | `{"tier", "displayName"}`   |
| `/internal/v1/api-keys/validate` | POST | Validate API key (Authorino callback) | None              | `{"key"}`         | `{"valid", "userId", "groups"}` |
