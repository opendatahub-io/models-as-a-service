...
// atomic UPDATE. Filters by username and/or subscription.
func (s *PostgresStore) BulkRevoke(ctx context.Context, username string, subscription string, tenant string) (int, error) {
...
func (s *PostgresStore) BulkRevokeCount(ctx context.Context, username string, subscription string, tenant string) (int, error) {
	if username == "" && subscription == "" {
		return 0, errors.New("at least one of username or subscription is required")
	}
	if tenant != s.tenantName {
		return 0, fmt.Errorf("%w: attempted to count keys for tenant %q but store is scoped to %q", ErrTenantMismatch, tenant, s.tenantName)
	}
