...
func TestBulkRevoke(t *testing.T) {
	...
	t.Run("BasicHappyPath", func(t *testing.T) {
		...
		count, err := store.BulkRevoke(ctx, "alice", "", "")
		require.NoError(t, err)
	...
	t.Run("NoKeysForUser", func(t *testing.T) {
		count, err := store.BulkRevoke(ctx, "nobody", "", "")
		require.NoError(t, err)
	...
	t.Run("MixedStatuses", func(t *testing.T) {
		...
		count, err := s.BulkRevoke(ctx, "carol", "", "")
		require.NoError(t, err)
	...
	t.Run("IdempotentSecondCall", func(t *testing.T) {
		...
		count, err := s.BulkRevoke(ctx, "dan", "", "")
		require.NoError(t, err)
	...
	t.Run("BySubscription", func(t *testing.T) {
		...
		count, err := s.BulkRevoke(ctx, "", "sub-alpha", "")
		require.NoError(t, err)
	...
	t.Run("ByUsernameAndSubscription", func(t *testing.T) {
		...
		count, err := s.BulkRevoke(ctx, "alice", "sub-x", "")
		require.NoError(t, err)
		assert.Equal(t, 1, count)
...
func TestBulkRevoke_TenantScoped(t *testing.T) {
	...
	count, err := store.BulkRevoke(ctx, "alice", "", "tenant-a")
	require.NoError(t, err)
	assert.Equal(t, 2, count)
...
func TestBulkRevoke_ExcludesExpiredKeys(t *testing.T) {
	...
	count, err := store.BulkRevokeCount(ctx, "alice", "", "tenant-a")
	require.NoError(t, err)
	assert.Equal(t, 2, count, "should count only active+permanent, not expired")
