...
// Supports dryRun=true to preview which keys would be revoked without mutating.
...
func (h *Handler) BulkRevokeAPIKeys(c *gin.Context) {
	...
	if req.DryRun {
		count, err := h.service.DryRunBulkRevokeCount(c.Request.Context(), req.Username, req.Subscription, user.Tenant)
		if err != nil {
	...
	count, err := h.service.BulkRevokeAPIKeys(c.Request.Context(), req.Username, req.Subscription, user.Tenant)
	if err != nil {
