...
func (m *Manager) FilterModelsByAccess(ctx context.Context, models []Model, authHeader string) []Model {
	...
	m.logger.V(1).Info("FilterModelsByAccess: validating access for models", "count", len(models))
	var out []Model
	var mu sync.Mutex
	...
	for i := range models {
		...
		g.Go(func() error {
			if discovered := m.fetchModelsWithRetry(ctx, authHeader, meta); discovered != nil {
				...
				mu.Unlock()
				m.logger.V(1).Info("FilterModelsByAccess: access granted", "model", model.ID, "endpoint", modelsEndpoint)
			} else {
