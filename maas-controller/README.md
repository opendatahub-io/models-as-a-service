...
**Config anchor.** The cluster-scoped `Config` named `default` is the Kubernetes controller owner for platform operands (maas-api workloads, gateway policies, cluster RBAC, and so on). Namespaced children in any namespace may reference this cluster-scoped owner. The namespace-scoped `MaasTenantConfig` (`default-tenant`) is also owned by `Config` so garbage collection removes it when the anchor is deleted. **`Config/default` is created by `LifecycleReconciler`** once the `maas-controller` Deployment is running (and recreated if accidentally deleted while the Deployment remains); the ODH operator may still create it via component reconcile. The manager bootstrap runnable creates **`AITenant/models-as-a-service`** once `Config` exists (shell CR only); **`LifecycleReconciler`** applies non-controller **`Config`→`AITenant`** and **`Config`→`MaasTenantConfig`** owner references. The `maas-controller` Deployment deliberately does not reference `Config`, so it can coordinate self-teardown and report completion after deleting the anchor. During component removal, the external operator annotates the Deployment with `maas.opendatahub.io/teardown-requested=true`; the lifecycle reconciler deletes all AITenants and waits for their cleanup, removes remaining finalizer-bearing MaaS CRs and tenant configs, deletes the `ai-tenants` infrastructure namespace and `Config/default`, and finally sets `maas.opendatahub.io/teardown-completed=true`. Per-tenant namespaces are retained by AITenant cleanup.
...
On a **fresh** cluster, applying the full bundle in one shot can apply workloads before MaaS CRDs are **Established**. **`./scripts/deploy.sh`** applies `deployment/base/maas-controller/crd` first and waits until every MaaS CRD is **Established**, then applies the rest of `deployment/base/maas-controller/default` (RBAC, Deployment, and so on). After the controller pod starts, **`LifecycleReconciler` creates `Config/default`** and links **`Config`→`AITenant/models-as-a-service`** and **`Config`→`default-tenant`**; the bootstrap runnable may create the default **`AITenant`** shell before those owner refs converge. The controller Deployment remains independent from `Config` so it can drive self-teardown.
