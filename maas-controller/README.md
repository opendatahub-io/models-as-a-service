...
**Config anchor.** The cluster-scoped `Config` named `default` is the Kubernetes controller owner for platform operands (maas-api workloads, gateway policies, cluster RBAC, and so on). Namespaced children in any namespace may reference this cluster-scoped owner. The namespace-scoped `MaasTenantConfig` (`default-tenant`) is also owned by `Config` so garbage collection removes it when the anchor is deleted. **`Config/default` is created by `LifecycleReconciler`** once the `maas-controller` Deployment is running; the ODH operator may still create it via component reconcile. The manager bootstrap runnable creates **`AITenant/models-as-a-service`** once `Config` exists (shell CR only); **`LifecycleReconciler`** applies non-controller **`Config`→`AITenant`** and **`Config`→`MaasTenantConfig`** owner references the same way it links **`Config`→`Deployment`**. When the operator removes the Config anchor, a cleanup finalizer keeps Config-owned maas-api and maas-controller resources alive while all AITenants and other finalizer-bearing MaaS CRs are deleted. After those finalizers finish, remaining MaasTenantConfig objects are removed and the Config finalizer is released for normal component garbage collection. The bootstrap runnable skips AITenant creation while Config is terminating, and the lifecycle reconciler does not recreate Config after it has observed coordinated teardown.
