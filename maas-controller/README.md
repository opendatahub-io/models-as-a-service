...
**`maas-parameters` ConfigMap ownership.** In ODH/RHOAI managed deployments the operator creates and owns the `maas-parameters` ConfigMap via `AppendOperatorInstallManifests` (sourced from `params.env`). The Tenant reconciler reads values from this ConfigMap (images, gateway coordinates, cache TTLs) but **must not overwrite it** — the operator is authoritative. In standalone kustomize deployments the ConfigMap is generated by the `configMapGenerator` in the overlay `kustomization.yaml`.
