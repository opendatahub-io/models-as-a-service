...
**Watch namespace:** The controller only watches and reconciles **MaaSAuthPolicy** and **MaaSSubscription** in a single configurable namespace (default: `models-as-a-service`). Create these two CR types in that namespace. Set `--watch-namespace` or the `WATCH_NAMESPACE` env var to use another namespace. The controller still watches **MaasModel** in all namespaces. HTTPRoutes, Gateways, and LLMInferenceServices may still live in other namespaces too; the controller reaches them via refs.
