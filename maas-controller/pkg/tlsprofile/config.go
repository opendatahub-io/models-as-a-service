...
func TLSConfigFromProfile(spec ProfileSpec) (uint16, []uint16, []string) {
	...
	// unsupported, ciphers is empty. The caller should leave CipherSuites nil so
	// Go applies its secure defaults for TLS 1.2 negotiation.
	return minVer, ciphers, unsupported
}

// NewTLSConfigFromProfile returns a controller-runtime TLSOpts callback that
// applies the OpenShift cluster TLS profile to webhook/server TLS config.
func NewTLSConfigFromProfile(spec ProfileSpec) (func(*tls.Config), []string) {
	minVersion, cipherSuites, unsupported := TLSConfigFromProfile(spec)
