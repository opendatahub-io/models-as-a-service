...
func TestMaaSSubscriptionReconciler_ManagedAnnotation(t *testing.T) {
	...
	maasSub := &maasv1alpha1.MaaSSubscription{
		ObjectMeta: metav1.ObjectMeta{Name: "sub-a", Namespace: namespace},
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := r.reconcileTokenRateLimitPolicies(context.Background(), logr.Discard(), maasSub); err != nil {
				t.Fatalf("reconcileTokenRateLimitPolicies: unexpected error: %v", err)
			}
...
// TestMaaSSubscriptionReconciler_DeleteAnnotation verifies that deleteModelTRLP respects
// the opt-out annotation: a TokenRateLimitPolicy with managed=false must not be deleted.
//
// NOTE: This test is expected to FAIL against the current implementation.
// deleteModelTRLP does not inspect annotations before deleting; it must be updated to
// skip resources where either managed annotation is "false".
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			c := fake.NewClientBuilder().
				WithScheme(scheme).
				WithRESTMapper(trlpTestRESTMapper()).
				WithObjects(existingTRLP).
				Build()

			r := &MaaSSubscriptionReconciler{Client: c, Scheme: scheme}
			if err := r.deleteModelTRLP(context.Background(), logr.Discard(), modelName); err != nil {
				t.Fatalf("deleteModelTRLP: unexpected error: %v", err)
			}
