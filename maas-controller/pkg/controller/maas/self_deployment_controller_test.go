...
func TestLifecycleReconciler_ControllerReplicasSetFromConfig(t *testing.T) {
	g := NewWithT(t)
	s := lifecycleTestScheme(t)
	...
	cfg := &maasv1alpha1.Config{
		ObjectMeta: metav1.ObjectMeta{
			Name: maasv1alpha1.ConfigInstanceName,
			UID:  types.UID("cfg-uid-replicas"),
		},
		Spec: maasv1alpha1.ConfigSpec{
			ControllerReplicas: ptr.To(int32(3)),
		},
	}
...
func TestLifecycleReconciler_ControllerReplicasNilPreservesDefault(t *testing.T) {
	g := NewWithT(t)
	s := lifecycleTestScheme(t)
	...
	cfg := &maasv1alpha1.Config{
		ObjectMeta: metav1.ObjectMeta{
			Name: maasv1alpha1.ConfigInstanceName,
			UID:  types.UID("cfg-uid-no-replicas"),
		},
		Spec: maasv1alpha1.ConfigSpec{},
	}
	...
	g.Expect(cl.Get(context.Background(), client.ObjectKey{Name: "maas-controller", Namespace: depNS}, &updated)).To(Succeed())
	g.Expect(updated.Spec.Replicas).NotTo(BeNil())
	g.Expect(*updated.Spec.Replicas).To(Equal(int32(1)))
}

func TestLifecycleReconciler_ControllerReplicasNoOpWhenAlreadyMatches(t *testing.T) {
	g := NewWithT(t)
	s := lifecycleTestScheme(t)
	...
	err := r.ensureControllerReplicas(context.Background(), ctrl.Log, cfg, types.NamespacedName{Name: "maas-controller", Namespace: depNS})
	g.Expect(err).NotTo(HaveOccurred())
