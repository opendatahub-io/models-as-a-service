...
func TestIsTenantNamespace_LabeledNamespace(t *testing.T) {
	ns := &corev1.Namespace{
		ObjectMeta: metav1.ObjectMeta{
			Name:   "team-a-maas",
			Labels: map[string]string{tenantreconcile.LabelManagedByAIGateway: "true"},
		},
	}
	c := fake.NewClientBuilder().WithScheme(scheme).WithObjects(ns).Build()
	if !isTenantNamespace(context.Background(), c, "team-a-maas", "models-as-a-service", true) {
		t.Error("AIGateway-labeled namespace should be recognized as tenant namespace")
	}
	if isTenantNamespace(context.Background(), c, "team-a-maas", "models-as-a-service", false) {
		t.Error("AIGateway-labeled namespace should be ignored while discovery is disabled")
	}
}
...
func TestMaaSAuthPolicyReconciler_ReconcilesTenantNamespace(t *testing.T) {
	...
	if err := c.Get(context.Background(), types.NamespacedName{Name: "maas-auth-" + modelName, Namespace: namespace}, authPolicy); err != nil {
		t.Errorf("AuthPolicy should be created for AIGateway-labeled tenant namespace: %v", err)
	}
}
