...
func (r *MaaSSubscriptionReconciler) reconcileTRLPForModel(ctx context.Context, log logr.Logger, modelNamespace, modelName string) error {
	...
	if denyCheck := buildMembershipCheck(allGroupNames, allUserNames); denyCheck != "" {
		limitsMap[fmt.Sprintf("deny-unsubscribed-%s", modelName)] = map[string]interface{}{
			"rates":    []interface{}{map[string]interface{}{"limit": int64(0), "window": "1m"}},
	...
	if len(subNames) > 0 {
		...
		limitsMap[fmt.Sprintf("deny-invalid-header-%s", modelName)] = map[string]interface{}{
			"rates":    []interface{}{map[string]interface{}{"limit": int64(0), "window": "1m"}},
			"when":     denyHeaderWhen,
			"counters": []interface{}{map[string]interface{}{"expression": "auth.identity.userid"}},
		}
	...
	if apierrors.IsNotFound(err) {
		...
		log.Info("TokenRateLimitPolicy created", "name", policyName, "model", modelName, "subscriptions", subNames)
	} else if err != nil {
		...
		if !isManaged(existing) {
			...
			if equality.Semantic.DeepEqual(snapshot.Object, existing.Object) {
				log.Info("TokenRateLimitPolicy unchanged, skipping update", "name", policyName, "model", modelNamespace+"/"+modelName)
			} else {
				if err := r.Update(ctx, existing); err != nil {
					return fmt.Errorf("failed to update TokenRateLimitPolicy for model %s/%s: %w", modelNamespace, modelName, err)
				}
				log.Info("TokenRateLimitPolicy updated", "name", policyName, "model", modelNamespace+"/"+modelName, "subscriptions", subNames)
			}
		}
	}
	return nil
