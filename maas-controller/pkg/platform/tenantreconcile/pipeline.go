...
// reconciler. Route names alone are not an ownership boundary. AppNamespace is the
// namespace resolved for this tenant by BuildPlatformParams, so the list remains
// tenant-scoped. Foreground deletion of the IPP Deployments completes before this
// runs, ensuring their in-process route writer has stopped.
func cleanupIPPExternalModelRoutes(ctx context.Context, c client.Client, params PlatformParams, log logr.Logger) error {
	...
	if err := c.List(ctx, routes, client.InNamespace(params.AppNamespace)); err != nil {
		return fmt.Errorf("list existing IPP HTTPRoutes in %s: %w", params.AppNamespace, err)
	}
	for i := range routes.Items {
		...
		if err := c.Get(ctx, client.ObjectKey{Namespace: params.AppNamespace, Name: modelName}, inference); err != nil {
			if apierrors.IsNotFound(err) {
				continue
			}
			return fmt.Errorf("get inference ExternalModel %s/%s for IPP-owned route %s: %w", params.AppNamespace, modelName, route.GetName(), err)
		}
