...
func TestCleanupIPPResources_DeletesOnlyOwnedInferenceRouteAfterWritersStop(t *testing.T) {
	const ns = "llm"
	params := PlatformParams{AppNamespace: ns, GatewayNamespace: "openshift-ingress", TenantIdentifier: "team-a"}
	scheme := praxisTestScheme(t)
...
func TestCleanupIPPResources_DefersRouteWhileIPPWriterExists(t *testing.T) {
	...
	params := PlatformParams{AppNamespace: ns, GatewayNamespace: gatewayNS, TenantIdentifier: tenantID}
	scheme := praxisTestScheme(t)
...
func TestCleanupIPPExternalModelRoutes_PreservesUnsafeOwnership(t *testing.T) {
	const ns = "llm"
	params := PlatformParams{AppNamespace: ns}
...
func TestCleanupIPPExternalModelRoutes_IsNamespaceScoped(t *testing.T) {
	...
	otherNamespaceRoute := ippOwnedExternalModelRoute("other", "demo", types.UID("uid"))
	cl := fake.NewClientBuilder().WithScheme(praxisTestScheme(t)).WithObjects(
		route, otherNamespaceRoute, ippExternalModel(ns, "demo", types.UID("uid")),
	).Build()
	require.NoError(t, cleanupIPPExternalModelRoutes(context.Background(), cl, PlatformParams{AppNamespace: ns}, logr.Discard()))

	got := &unstructured.Unstructured{}
	got.SetGroupVersionKind(GVKHTTPRoute)
	assert.True(t, apierrors.IsNotFound(cl.Get(context.Background(), client.ObjectKey{Namespace: ns, Name: "demo"}, got)))
	assert.NoError(t, cl.Get(context.Background(), client.ObjectKey{Namespace: "other", Name: "demo"}, got))
}
...
func TestCleanupIPPExternalModelRoutesPropagatesDeletePreconditionFailure(t *testing.T) {
	...
	err := cleanupIPPExternalModelRoutes(context.Background(), cl, PlatformParams{AppNamespace: ns}, logr.Discard())
