...
echo "=== Installing MaaS controller (deploy: $CONTROLLER_NAMESPACE, watch: $WATCH_NAMESPACE) ==="

# Validate deploy namespace exists (do not create it)
if ! kubectl get namespace "$CONTROLLER_NAMESPACE" &>/dev/null; then
  echo "Error: deploy namespace $CONTROLLER_NAMESPACE does not exist. Create it first (e.g. via Open Data Hub operator) or choose another namespace."
  exit 1
fi
# Ensure watch namespace exists (create if missing)
if ! kubectl get namespace "$WATCH_NAMESPACE" &>/dev/null; then
  echo "Creating watch namespace $WATCH_NAMESPACE"
